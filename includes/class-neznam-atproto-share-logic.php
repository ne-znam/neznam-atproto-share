...
class Neznam_Atproto_Share_Logic {
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->did           = $this->getDid( $this->handle );
	}
	...
	public function cron() {
		...
		while ( $q->have_posts() ) {
			$q->the_post();
			$this->postMessage( $q->post );
		}
	}

	public function postMessage( WP_Post $post ): void {
		if ( ! $this->access_token ) {
			$this->authorize();
		}
		$image_path      = get_attached_file( get_post_thumbnail_id( $post->ID ) );
		$blob            = $this->uploadBlob( $image_path );
		...
		$body            = wp_remote_post( trailingslashit( $this->url ) . 'xrpc/com.atproto.repo.createRecord', [
			'headers' => [
	...
	private function authorize(): void {
		...
		$body                = wp_remote_post( trailingslashit( $this->url ) . 'xrpc/com.atproto.server.createSession', [
			'headers' => [
				'Content-Type' => 'application/json'
			],
			'body'    => json_encode( [
	...
	private function refreshToken(): void {
		$body = wp_remote_post( trailingslashit( $this->url ) . 'xrpc/com.atproto.server.refreshSession', [
			'headers' => [
				'Content-Type' => 'application/json'
			],
			'body'    => json_encode( [
	...
	private function uploadBlob( string $path ): array {
		if ( ! $path ) {
			return [];
		}
		$body = wp_remote_post( trailingslashit( $this->url ) . 'xrpc/com.atproto.repo.uploadBlob', [
			'headers' => [
				'Authorization' => 'Bearer ' . $this->access_token,
				'Content-Type'  => 'image/*'
			],
